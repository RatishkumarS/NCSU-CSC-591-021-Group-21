def generate_ascii_table(csv_file_path):
    ...
    class_percentage = {key: (value / total_rows) * 100 for key, value in class_counts.items()}

    # Determine the maximum width for formatting
    max_width_class = max(len("Class"), max(len(class_label) for class_label in class_counts))
    max_width_rows = max(len("Number of Rows"), max(len(str(count)) for count in class_counts.values()))
    max_width_percentage = max(len("Class Percentage"), max(len(f"{percentage:.2f}%") for percentage in class_percentage.values()))

    # Generate and print ASCII table
    print(f"{ 'Class':<{max_width_class}} | {'Number of Rows':<{max_width_rows}} | {'Class Percentage':<{max_width_percentage}}")
    print("-" * (max_width_class + max_width_rows + max_width_percentage + 7))

    for class_label, count in class_counts.items():
        percentage = class_percentage[class_label]
        print(f"{class_label:<{max_width_class}} | {count:<{max_width_rows}} | {percentage:.2f}%")
